...
Streamlit + Supabase PostgreSQL tek dosya uygulama. PostgreSQL bağlantısı psycopg3 ile yapılır.
...
class PostgresConnection:
    def __init__(self):
        ...
        self._conn = psycopg.connect(with_sslmode_require(database_url))
...
def read_df(query: str, params: Iterable = ()) -> pd.DataFrame:
    ...
    if not database_url:
        return pd.DataFrame()
    with closing(psycopg.connect(with_sslmode_require(database_url))) as conn:
